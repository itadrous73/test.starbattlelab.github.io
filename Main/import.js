...
// all CSS styles, with a reset rule at the top for complete isolation.
    const customCss = `
		...
		.sbi-cell-invalid {
		  ...
		  /* formerly Tailwind red-400 */
		}
		.sbi-tool-btn.sbi-active {
		  background-color: #2563eb;
		  /* formerly Tailwind blue-600 */
		  border-color: #2563eb;
		  color: #ffffff;
		}
		...
		#eraserBtn.sbi-active {
		  background-color: #ef4444; /* red-500 */
		  border-color: #ef4444;
		  color: #ffffff; /* This will make the SVG icon white */
		}
		...
		<div id="importerContainer" style="position: fixed; inset: 0; background-color: rgba(0, 0, 0, 0.7); display: none; align-items: center; justify-content: center; z-index: 100;">
			<div id="sbi-main-content" style="width: 51%; background-color: #1f2937; padding: 2rem; border-radius: 0.5rem; box-shadow: 0 10px 15px rgba(0,0,0,0.5); position: relative; border: 1px solid #374151;">
				...
				<div style="display: flex; gap: 1rem;">
					...
					<button id="photoImportBtn" style="width: 100%; background-color: #4b5563; color: white; padding: 0.75rem 1rem; border-radius: 0.375rem; transition: background-color 0.2s; border: none;">Import from Photo</button>
				</div>
			</div>
			<!-- Photo Import Modal -->
...
`;
	...
	const GridManager = {
		...
		init(size, container) {
			...
			this.container.style.gridTemplateColumns = `repeat(${size}, 35px)`;
			this.container.style.gridTemplateRows = `repeat(${size}, 35px)`;
